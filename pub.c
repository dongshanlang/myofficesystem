...
//将HTTP GET请求中的转义符号转化为标准字符,注意，空格被转义为'+'号
//%E6%9D%8E%E7%A7%80%E6%95%8F&sex=m&studentID=201428006812040&wd=13373882639&sports=a&sports=c&sports=d&sports=e&sports=f
void httpstr2stdstr_sportsmeeting(const char *httpstr, char **sports)
{
	int index = 0;
	int i;
	char *p = NULL;

	if((p = strstr(httpstr, "name=")) != NULL)
	{
		...
		httpstr2stdstr(cut, stdstr);
		strcpy(sports[0],stdstr);
	}

	char sextemp[] = "sex=";
	if((p = strstr(httpstr, sextemp)) != NULL)
	{
		char temp = p[4];
		if (temp == 'm')
		{
	...
	char studentIDtemp[] = "studentID=";
	if((p = strstr(httpstr, studentIDtemp)) !=NULL)
	{
		index = strlen(studentIDtemp);
		for(i = 0; i<15; i++)
		{
			sports[2][i] = p[index++];
		}
	...
	char phonetemp[] = "phone=";
	if((p = strstr(httpstr, phonetemp)) != NULL)
	{
		index = strlen(phonetemp);
		for(i = 0; i<11; i++)
		{
			sports[3][i] = p[index++];
		}
	...
	index = 4;
	if((p = strstr(httpstr, "sportsa=")) != NULL)
	{
		strcpy(sports[index++], "60米跑" );
	...
	if((p = strstr(httpstr, "sportsb=")) != NULL)
	{
		strcpy(sports[index++], "60米二人三足");
	}else
	{
		strcpy(sports[index++], "" );
	}
	if((p = strstr(httpstr, "sportsc=")) != NULL)
	{
		strcpy(sports[index++], "篮球射门");
	}else
	{
		strcpy(sports[index++], "" );
	}
	if((p = strstr(httpstr, "sportsd=")) != NULL)
	{
		strcpy(sports[index++], "铅球");
	}else
	{
		strcpy(sports[index++], "" );
	}
	if((p = strstr(httpstr, "sportsed=")) != NULL)
	{
		strcpy(sports[index++], "一分钟跳绳");
	}else
	{
		strcpy(sports[index++], "" );
	}
	if((p = strstr(httpstr, "sportsf=")) != NULL)
	{
		strcpy(sports[index++], "立定跳远");
	}else
	{
		strcpy(sports[index++], "" );
	}
...
void socket_accept(int st)
{
	...
	while (1)//循环执行accept
	{
		...
		//accept函数阻塞，只到有client端连接到达，或者accept错误返回
